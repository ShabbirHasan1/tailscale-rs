...
pub trait PeerLookup: Send + Sync {
    ...
    /// exist yet.
    fn key_to_id(&self, key: &ts_keys::NodePublicKey) -> ts_transport::PeerId;
...
impl<T> PeerLookup for Arc<T>
where
    T: PeerLookup,
{
    fn key_to_id(&self, key: &ts_keys::NodePublicKey) -> ts_transport::PeerId {
        self.as_ref().key_to_id(key)
    }

    fn id_to_key(&self, id: ts_transport::PeerId) -> Option<ts_keys::NodePublicKey> {
        self.as_ref().id_to_key(id)
    }
}

impl<T> PeerLookup for Box<T>
where
    T: PeerLookup,
{
    fn key_to_id(&self, key: &ts_keys::NodePublicKey) -> ts_transport::PeerId {
...
impl PeerLookup for DummyStaticLookup {
    fn key_to_id(&self, key: &ts_keys::NodePublicKey) -> ts_transport::PeerId {
        ...
        if let Some((id, _)) = mp.iter().enumerate().find(|(_id, k)| *k == key) {
            return ts_transport::PeerId(id as _);
        }

        let id = mp.len();
        mp.push(*key);

        ts_transport::PeerId(id as _)
    }
